...
usageFunc() {
echo ""
echo "Usage:  $0 [-f input_pdb_file] [-s input_linker_shortest] [-l input_linker_longest] [-c input_contig_map]"
echo ""
...
while getopts "f:s:l:c:h " option; do
  case $option in
    f) input_pdb_file="$OPTARG" ;;
    s) input_linker_shortest="$OPTARG" ;;
    l) input_linker_longest="$OPTARG" ;;
    c) input_contig_map="$OPTARG" ;;
    h) usageFunc
       exit 1 ;;
    *) usageFunc 
       exit 1 ;;
...
# Confirm correct conda env used
